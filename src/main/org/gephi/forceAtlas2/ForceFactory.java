...
public class ForceFactory {
    ...
    private static ForceFactory builder = new ForceFactory();

    private ForceFactory() {
    }

    public static ForceFactory getForceFactory() {
        return builder;
    }
    ...
    public interface CustomForce {

        void apply(Node[] nodes, Edge[] edges); // Model for applying custom forces
    }
    ...
    private class linRepulsion extends RepulsionForce {
        ...
        public void apply(Node n1, Node n2) {
            ...
            if (distance > 0) {
                ...
                double factor = coefficient * n1.getMass() * n2.getMass() / distance / distance;

                n1.shiftDx(xDist * factor);
                n1.shiftDy(yDist * factor);
                n2.shiftDx(-xDist * factor);
                n2.shiftDy(-yDist * factor);
            }
        }
        ...
        public void apply(Node n, Region r) {
            ...
            if (distance > 0) {
                // NB: factor = force / distance
                double factor = coefficient * n.getMass() * r.getMass() / distance / distance;

                n.shiftDx(xDist * factor);
                n.shiftDy(yDist * factor);
            }
        }
    ...
    private class logAttraction_degreeDistributed extends AttractionForce {
        ...
        public void apply(Node n1, Node n2, double e) {
            ...
            if (distance > 0) {
                ...
                // NB: factor = force / distance
                double factor = -coefficient * e * Math.log(1 + distance) / distance / n1.getMass();
