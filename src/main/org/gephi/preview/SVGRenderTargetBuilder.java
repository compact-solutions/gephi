...
public class SVGRenderTargetBuilder implements RenderTargetBuilder {
    ...
    public static class SVGRenderTargetImpl implements SVGTarget {
        ...
        private final String topElementsWrapperId = "top-elements-container";
        private final Element topElementsWrapper;
        private final String metadataWrapperName = "metadata";
        private Element metadataWrapper;
        private float scaleRatio = 1f;
        private final Map<String, Element> topElements = new HashMap<>();
        private final Map<String, Element> metadataTopElements = new HashMap<>();
        ...
        public SVGRenderTargetImpl(CanvasSize cs, boolean scaleStrokes) {
            ...
            topElementsWrapper = createElement("g");
            topElementsWrapper.setAttribute("id", topElementsWrapperId);
            svgRoot.appendChild(topElementsWrapper);
        ...
        public Element getTopElement(String name) {
            ...
            if (topElement == null) {
                ...
                topElementsWrapper.appendChild(topElement);
            }
            return topElement;
        }

        @Override
        public Element getMetadataTopElement(String name) {
            if (metadataWrapper == null) {
                metadataWrapper = document.createElement(metadataWrapperName);
                document.getDocumentElement().appendChild(metadataWrapper);
            }
            Element topElement = metadataTopElements.get(name);
            if (topElement == null) {
                topElement = document.createElement(name);
                metadataTopElements.put(name, topElement);
                metadataWrapper.appendChild(topElement);
            }
            return topElement;
        }
